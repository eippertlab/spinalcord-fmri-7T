...
for subject in {1..62}; do
	...
	out_dir=$project_dir/derivatives/sub-${sub}/anat_t2s
	...
	if [ $combine_images = 1 ]; then
		...
		# combine echos using root mean square
	    	...
	    	if [[ $subject == 29 ]]; then
	    		fslmerge -t rms_combined_merged.nii.gz rms_combined_run-1.nii.gz rms_combined_run-2.nii.gz
	    	else
	...
	# -------------------------------------------------------------------------------------------
    	...
    	# -------------------------------------------------------------------------------------------
    	# check these segmentations also manually
    	# -------------------------------------------------------------------------------------------
	...
	if [ $register_epi = 1 ]; then
		...
		if [[ "$subject" == 29 || "$subject" == 60 ]]; then
    		...
    		cd $out_dir
    		echo "Registering target EPI and MEGRE images"
		...
		# when assessing how distorted the EPI is in comparison --> for Dice coefficient
    		...
    		-owarp warp_MEGRE_RMS_MOCO_MEAN2MOCO_MEAN_translation.nii.gz \
    		-owarpinv warp_MOCO_MEAN2MEGRE_RMS_MOCO_MEAN_translation.nii.gz
	...
	if [ $warp_masks = 1 ]; then
		...
		-d $func_dir/MOCO_MEAN.nii.gz \
		-w warp_MEGRE_RMS_MOCO_MEAN2MOCO_MEAN_translation.nii.gz \
		-x nn \
